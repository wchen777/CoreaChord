...
public class AnalyzerApplication {
  ...
  // array of circle of fifths to determine complexity score, get root enum to "index" in circle of fiths
  private HashMap<CoreaApplication.Root, Integer> circleOfFifths = new HashMap<>();


  private final int NUMROOTS = 12;
  ...
  public void windowTwoCadences(List<GeneratedChord> chordProg, List<GeneratedCadence> cadences) {
    ...
    while (i < chordProg.size() - 1) {
      ...
      if (CadenceCalculator.isHalfStepDown(c1, c2) && CadenceCalculator.isDominant7(c1)) {
        cadences.add(new GeneratedCadence(Cadence.subV_I, currMeasure, end, c2.getRoot(), progression));
      }

      // if Imaj7 to Im7
      if (CadenceCalculator.isSameRoot(c1, c2) && CadenceCalculator.isMajorToMinor(c1, c2)){
        cadences.add(new GeneratedCadence(Cadence.Imaj7_Im7, currMeasure, end, c2.getRoot(), progression));
      }
      ...
      if (CadenceCalculator.isDominant7(c1)
        ...
        cadences.add(new GeneratedCadence(Cadence.bVII_I, currMeasure, end, c2.getRoot(), progression));
      }
  ...
  public void windowThreeCadences(List<GeneratedChord> chordProg, List<GeneratedCadence> cadences) {
    ...
    while (i < chordProg.size() - 2) {
      ...
      // get end measure
      int end = currMeasure + chordProg.get(i + 1).getChordlength() + chordProg.get(i + 2).getChordlength();

      String progression = c1.toFormattedString() + " -> " + c2.toFormattedString() + " -> " + c3.toFormattedString();

      // create IIVI cadence
      if (CadenceCalculator.isIIVI(c1, c2, c3)) {
        cadences.add(new GeneratedCadence(Cadence.II_V_I, currMeasure, end, c3.getRoot(), progression));
      }
  ...
  public List<ComplexityScore> getComplexityScore(List<Chord> chordsRaw) {
    ...
    while (i < chordsRaw.size() - 2) {
      ...
      double firstScore = circularDistance(circleOfFifths.get(c2.getRoot()), circleOfFifths.get(c1.getRoot()));
      double secondScore = circularDistance(circleOfFifths.get(c3.getRoot()), circleOfFifths.get(c2.getRoot()));

      // 22 is the max value of these two scores added together
      double newScore = (firstScore + secondScore) / NUMROOTS;
  ...
  public Tuple<List<ComplexityScore>,List<GeneratedCadence>> generate(List<GeneratedChord> chordProg) {
